...
# westonite opens --log in append mode (main.c fopen "a"): stale logs
# from a previous run in the same container would satisfy (or trip) the
# marker greps below — start clean.
rm -f /tmp/w1.log /tmp/w2.log /tmp/w3.log
...
# P3 (no-panel) guarantee, both shells: no helper client may be spawned.
