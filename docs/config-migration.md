...
- The **search order**: `$XDG_CONFIG_HOME`, then `$HOME/.config`, then
  `$XDG_CONFIG_DIRS` (default `/etc/xdg`); `--config=PATH` and
  `--no-config` still work.  Only the file *name* changes:
  `westonite.toml` instead of `westonite.ini`.
...
| `backends=drm,vnc` | `backends = ["drm", "vnc"]` | comma lists become arrays |
...
| `background-color=0xff002244` | `background-color = "0xff002244"` | quoted string, hex spelling kept |
...
tree after the file and before dedicated flags.  Unknown flags are
rejected by clap before startup (stderr), not logged as
`unhandled option:`.
