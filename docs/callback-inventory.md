...
**28** `wl_listener` struct fields, **23** signal/destroy-listener
attach sites for those fields (the 24th `wl_signal_add`-family call in
the tree is `weston_compositor_add_destroy_listener_once` at
`shell.c:2184`, counted under the compositor-destroy row), **5** binding
registrations with our own handlers + `weston_install_debug_key_binding`
(handler is libweston's) = the plan's "6 binding registrations", and the
`weston_desktop_api` vtable (14 struct entries; the C shell installs 10,
the 4 unfilled ones have "not installed" rows).
...
| L2 | `wet_head_tracker.resized_listener` | `simple_heads_output_sharing_resize` (2581) | `main.c:2640` | `output_resized_signal` | deferred | mirror-of resize propagation (R2b) | — |
...
| L5 | `wet_compositor.screenshot_auth` | `screenshot_allow_all` (4396) | via `weston_compositor_add_screenshot_authority` (4577) | auth request | sync (out-param decision; answers from config value) | — | — |
...
| L7 | `wet_process_context.primary_client_destroyed` | `handle_primary_client_destroyed` (926) | `main.c:4704` | client destroy | deferred | shutdown-on-client-exit policy | — |
...
### desktop-shell/shell.c + shell.h (18 fields)
...
| L19 | `shell_seat.keyboard_focus_listener` | `shell_seat_handle_keyboard_focus` | `shell.c:1149` (from caps_changed) | keyboard `focus_signal` | deferred | activation bookkeeping; C walks `focused_surface` invalidation (1277-1287) | — |
| L20 | `workspace.seat_destroyed_listener` | `seat_destroyed` (477) | focus-state path | seat destroy | sync+inval | — | — |
...
| L26 | `desktop_shell.pointer_focus_listener` | (shell-wide pointer focus) | shell init | pointer `focus_signal` | deferred | busy-cursor policy | — |
...
plain deferred notifications.  Field count: 7 + 3 + 18 = 28.)*

## 2. Bindings (6 registrations)
...
## 3. `weston_desktop_api` (14 struct entries; 10 installed at `shell.c:1607-1619`)
