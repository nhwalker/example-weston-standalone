...
impl ShellHost for MockHost {
    ...
    fn activate_input(
        ...
        self.tracked_for.borrow().get(&id).copied()
    }
...
fn shutdown_destroys_every_surface_and_clears_state() {
    ...
    let mut destroyed = host.destroyed.borrow().clone();
    destroyed.sort();
    let mut expect = vec![forge::ds(0), forge::ds(1), forge::ds(2)];
    expect.sort();
    assert_eq!(destroyed, expect);
